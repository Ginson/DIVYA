...
class PropertiesPanel(QWidget):
    ...
    def set_node(self, node_item):
        ...
        for param_name, param_value in base_node.param_values.items():
            ...
            self.form_layout.addRow(QLabel(param_name.replace("_", " ").title()), editor)
    ...
    def on_param_changed(self, param_name, new_value):
        ...
        if self.current_node:
            ...
            try:
                original_type = type(self.current_node.base_node.param_values[param_name])
                converted_value = original_type(new_value)
                self.current_node.base_node.set_param_value(param_name, converted_value)
                print(f"Set '{param_name}' to {converted_value}")
            except (ValueError, TypeError) as e:
                print(f"Invalid value for '{param_name}': {new_value}. Error: {e}")
