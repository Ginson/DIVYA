...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QHBoxLayout, QSplitter, QGraphicsView, QGraphicsScene, QFileDialog
)
...
class MainWindow(QMainWindow):
    ...
    def add_edge_to_graph(self, start_socket, end_socket):
        ...
        print(f"UI: Adding edge from {source_node_id}:{source_output_name} to {target_node_id}:{target_input_name}")
        self.graph.add_edge(source_node_id, source_output_name, target_node_id, target_input_name)
        self.execute_graph()

    def save_pipeline(self):
        """Saves the current pipeline to a YAML file."""
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Pipeline", "", "YAML Files (*.yaml *.yml)")
        if file_path:
    ...
    def load_pipeline(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Load Pipeline", "", "YAML Files (*.yaml *.yml)")
        if file_path:
            ...
            for node_id, base_node in self.graph.nodes.items():
                ...
                base_node.ui_node = ui_node # Link the base node to the ui node
                self.graph_view.scene.addItem(ui_node)
                if hasattr(base_node, 'temp_pos'):
                    ui_node.setPos(base_node.temp_pos[0], base_node.temp_pos[1])

                # Connect display node signal if it's a DisplayNode
                if base_node.name == "Display Image":
                    base_node.image_processed.connect(self.image_display.set_image)
            ...
            for edge_data in graph_data['edges']:
                ...
                if source_node and target_node:
                    ...
                    for socket in source_node.ui_node.outputs:
                        if socket.socket_name == edge_data['source_output_name']:
                            source_socket = socket
                            break
                    
                    target_socket = None
                    for socket in target_node.ui_node.inputs:
                        if socket.socket_name == edge_data['target_input_name']:
