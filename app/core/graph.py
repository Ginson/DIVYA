...
class Graph:
    ...
    def add_edge(
        ...
        if (
            source_node_id not in self.nodes
            or target_node_id not in self.nodes
        ):
            raise KeyError(
                "Cannot create edge: source or target node not found in graph."
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Graph with {len(self.nodes)} nodes and {len(self.edges)} edges."
        )
            ...
            nodes_data.append(
                {
                    ...
                    "pos": (
                        [node.ui_node.x(), node.ui_node.y()]
                        if hasattr(node, "ui_node")
                        else [0, 0]
                    ),
                }
            )
